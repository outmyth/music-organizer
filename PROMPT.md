...
- 生成 M3U8 播放列表（Sony HAP-Z1ES / Walkman / Chord Poly 兼容）
- 生成 `music_index.json` 索引
...
└── out/                 ← 输出（整盘拷到 SD 卡根目录）
    ...
    │   └── Playlists/   ← Sony WM1A 专用播放列表目录（内容与 playlists/ 相同）
...
**命名规范**：`<Category>_<Name>.m3u`——下划线分隔类别和名称，连字符 ` - ` 分隔名称内部字段（如 `Album_Miles Davis - Kind of Blue.m3u`）。扁平目录设计是因为 Lotoo PAW Gold 2017 的扫描器不递归子目录。
...
每次运行结束时，脚本：
1. 收集本次所有写入目标路径
2. 扫描 `out/MUSIC/` 下所有音频文件和 `folder.jpg`（跳过 `MUSIC/Playlists/`）
...
所有 `.m3u` 每次全量重建（先删除旧文件），**每张播放列表同时写到 `playlists/` 和 `MUSIC/Playlists/`**（后者是 Sony WM1A 独家扫描路径）。

**路径语义**：`os.path.relpath(track, playlist_file.parent)`——相对于**播放列表文件自身所在目录**（标准 M3U 语义，RFC 8216），不是相对于 SD 卡根。
...
- 编码：UTF-8 + BOM（Lotoo 和老版 Chord Poly 需要 BOM 才能正确识别中文）
- 行尾：CRLF（Sony / Lotoo 固件偏好）

**兼容性说明**：

| 设备 | 固件坑 | 脚本的处理 |
|------|--------|-----------|
| Sony WM1A / WM1ZM2 | 只索引 `/MUSIC/` 大写目录；只扫描 `MUSIC/Playlists/` | 音乐目录用 `MUSIC/`；播放列表同时写到 `MUSIC/Playlists/` |
| Lotoo PAW Gold 2017 | 播放列表扫描器不递归子目录 | 扁平 `<Category>_<Name>.m3u` 命名 |
| Chord Poly (GoFigure) | `.m3u8` 不稳；路径遵循标准 M3U | 扩展名用 `.m3u`；路径相对播放列表自身 |
| FAT32 / exFAT | 半/全角冒号、单引号、感叹号等会让扫描器出错 | `sanitize()` 清理非法字符 |

**Chord Poly MPD 模式的例外**：MPD 按 `music_directory` 解析路径，需要在 MPD 配置里设置 `music_directory = /path/to/out/MUSIC/`，这属于外部配置，不在脚本范围内。
